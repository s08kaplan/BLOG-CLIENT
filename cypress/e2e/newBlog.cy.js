describe('newBlog', () => {
   ...
   it('adding new blog test', () => {
     ...
     cy.get('[data-test="avatar"]').should('be.visible').click({force: true})
     cy.get('[data-test="newBlogLink"]').should('be.visible').click({force: true})
     cy.url().should('include','/new-blog')
     cy.get('[data-test="newBlogTitle"]', { timeout: 15000 }).should('exist').should('be.visible').type("Cypress Test")
   //   cy.get('div.ql-editor.ql-blank').should('be.visible').type("Cypress Test Quill Content")
   //   cy.get('[data-test="quillEditor"]').should('be.visible').type("Cypress Test Quill Content")
   
   cy.typeInQuill('[data-test="quillEditor"]', 'Cypress Test Quill Content!');
   cy.get('[data-test="quillEditor"] .ql-editor')
     .should('contain', 'Cypress Test Quill Content!');
